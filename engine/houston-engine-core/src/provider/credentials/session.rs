...
pub fn take_import_session(
    ...
    let session = map.remove(session_id).ok_or_else(|| {
...
mod tests {
    ...
    fn session_single_use() {
        ...
        let secret = take_import_session(&info.session_id, CredentialProvider::OpenAi).unwrap();
        assert_eq!(secret.as_bytes().len(), 32);
        assert!(take_import_session(&info.session_id, CredentialProvider::OpenAi).is_err());
    }
}
