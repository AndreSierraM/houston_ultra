...
| POST | `/v1/providers/openrouter/credentials` | Write `OPENROUTER_API_KEY` to `~/.houston/openrouter/.env` (atomic, mode 0600). Body: `{ apiKey }`. Houston injects the env var into Codex subprocesses at spawn; OpenRouter has no CLI login. |
